...
//------------------------DB접속------------------------------
	...
	char message2[200];
	sprintf(message2, "%s 의 전원이 복구되었습니다.", row_4[0]);
	strcpy(sContent,message2);
	...
	sprintf(qbuf_7, "UPDATE rec SET blackoutstop = '%04d-%02d-%02d  %02d:%02d:%02d' WHERE blackoutstop = '0'", today->tm_year + 1900, today->tm_mon + 1, today->tm_mday, today->tm_hour, today->tm_min, today->tm_sec);
